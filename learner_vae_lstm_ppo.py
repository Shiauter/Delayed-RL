...
from actor_vae_lstm_ppo import Actor
...
class Learner:
    ...
    def _get_start_o(self, s, h):
        ...
        for i in range(len(s)):
            o_out, h = self.actor.rnn(s[i], h)
            o.append(o_out)
        return torch.cat(o, dim=1)

    def _one_step_pred(self, s, a_lsts, h, s_truth):
        # s       -> (batch=1, ep_len - delay, s_size) # current s
        # a_lsts  -> (batch=1, ep_len - delay, delay)
        # h       -> (seq_len=1, batch=1, hidden_size)
        # s_truth -> (delay, ep_len - delay, s_size) # for delay prediction

        all_o = self._get_start_o(s, h)

        # predicting one step future state using true state
        a_lst = torch.split(a_lsts, 1, dim=-1) # split into actions
        kld_loss, nll_loss, phi_x_truth, phi_z_truth, mse_loss, pred_s, dec_std = self.actor.pred_model.reconstruct(s_truth, s, a_lst[0], all_o)
    ...
    def _cal_pred_model_loss(self, s, a_lsts, first_hidden):
        ...
        if self.p_iters > 0:
            s_truth = s[:, 1:]
            kld_loss, nll_loss, mse_loss, dec_std_mean, dec_std_max, dec_std_min = self._one_step_pred(s[:, :-1], a_lsts[:, :-1], first_hidden, s_truth)
        return kld_loss, nll_loss, mse_loss, dec_std_mean, dec_std_max, dec_std_min
    # endregion
    ...
    def _make_pi_and_critic(self, s, a_lsts, first_hidden):
        ...
        for i in range(len(s)):
            ...
            _, pi, h_out, _, v = self.actor.sample_action(s[i], a_lsts[i], h_in)
            h_in = h_out
