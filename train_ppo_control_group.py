...
if __name__ == "__main__":
    ...
    if do_save:
        log_dir, saved_folder = config.log_dir, config.saved_folder
        check_saves_exist(log_dir, saved_folder)
    ...
    for ep in range(1, config.K_epoch_training + 1):
        ...
        do_record = ep % config.record_interval == 0 and config.do_record
        if do_record:
